...
def solve(filename, num_attempts):
    # Parse input
    boxes = []
    ...
    # Try to connect the first num_attempts pairs
    uf = UnionFind(n)
    successful = 0
    for attempt in range(num_attempts):
        ...
        if uf.union(i, j):
            successful += 1
            print(f"  Attempt {attempt+1}: Connected {i} and {j} (successful)")
        else:
            print(f"  Attempt {attempt+1}: {i} and {j} already connected (skipped)")

    print(f"\nMade {successful} successful connections out of {num_attempts} attempts")
...
if __name__ == "__main__":
    # Test with example (10 attempts for 20 boxes)
